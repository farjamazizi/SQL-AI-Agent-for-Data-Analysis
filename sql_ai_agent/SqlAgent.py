...
class QueryOutput:
    ...
    review_original_query: str | None = None
    review_output: str | None = None
    review_corrected_query: str | None = None
...
def extract_sql_from_llm_output(llm_output) -> str:
    """Extract SQL text from an LLM message, including markdown SQL blocks."""
    if pq.is_markdown_code_chunk(text=llm_output.content):
        return pq.extract_code_from_markdown(markdown_text=llm_output.content)
...
def query_processing(
    ...
    prompt=None,
    review_status=None,
    review_original_query=None,
    review_output=None,
    review_corrected_query=None,
):
        ...
        try:
            ...
            if not is_valid:
                ...
                return QueryOutput(
                    ...
                    review_original_query=review_original_query,
                    review_output=review_output,
                    review_corrected_query=review_corrected_query,
                )
...
class SqlAgent:
    def __init__(
        ...
        Args:
            ...
            sql_review: Enable LLM-based SQL semantic review before execution (default: False)
        """
        ...
        self.con = con
        self.sql_review_enabled = sql_review
        ...
        self.sql_review_prompt_template = ph.sql_review_prompt_template()
        self.sql_review_chain = self.sql_review_prompt_template | self.llm
        ...
        if self.logger:
            self.logger.info(
                ...
                extra={
                    ...
                    "sql_review_enabled": self.sql_review_enabled,
...
Schema: {self.schema}"""
        ...
        review_original_query = None
        review_output_text = None
        review_corrected_query = None

        if self.sql_review_enabled:
            original_query = extract_sql_from_llm_output(llm_output)
            review_original_query = original_query
            review_output = sql_review_agent(
                chain=self.sql_review_chain,
                question=question,
                tbl_name=self.tbl_name,
                db_type=self.db_type,
                schema=self.schema,
            ...
            review_content = review_output.content.strip()
            review_output_text = review_content

            if review_content.upper() != "APPROVED":
                reviewed_query = extract_sql_from_llm_output(review_output)
                if reviewed_query:
                    review_status = "corrected"
                    review_corrected_query = reviewed_query
                    llm_output = AIMessage(content=reviewed_query)
                    prompt_text += (
                        "\n\nSQL Review: reviewer returned a corrected query."
                    )
                    if self.logger:
                        self.logger.info(
                            "SQL review corrected generated query",
                            extra={
                                "operation_type": "sql_review",
                                "original_query": original_query,
                                "reviewed_query": reviewed_query,
                            },
                        )
                    elif verbose:
                        print("ℹ️  SQL review corrected the generated query.")
            else:
                review_status = "approved"
                prompt_text += "\n\nSQL Review: APPROVED"
                if self.logger:
                    self.logger.info(
                        "SQL review approved generated query",
                        extra={
                            "operation_type": "sql_review",
                            "query": original_query,
                        },
                    ...
                    print("✓ SQL review approved the generated query.")
        ...
        query = query_processing(
            ...
            review_status=review_status,
            review_original_query=review_original_query,
            review_output=review_output_text,
            review_corrected_query=review_corrected_query,
        )
