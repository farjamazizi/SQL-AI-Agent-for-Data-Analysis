...
For local WSL usage, PostgreSQL is expected at `localhost:5432` and credentials
are usually loaded from the project `.env` file.
...
psql -h localhost -U postgres -d my_db -c "SELECT 1"
```

Use `db_host="localhost"` for local notebooks and command-line tests. The
hostname `postgres` only works inside the original Docker Compose network.
