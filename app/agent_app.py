...
@st.cache_resource
def init_database_connection(db_type, host=None, port=None):
...
def display_query_result(result):
    ...
    if getattr(result, "review_status", None) == "approved":
        st.success("🔎 SQL Review Agent approved the generated query.")
        with st.expander("🔎 SQL Review Agent Output", expanded=False):
            st.text(result.review_output or "APPROVED")
    elif getattr(result, "review_status", None) == "corrected":
        st.warning("🔎 SQL Review Agent corrected the generated query before execution.")
        with st.expander("🔎 SQL Review Agent Output", expanded=False):
            if result.review_original_query:
                st.caption("Original generated SQL")
                st.code(format_sql_query(result.review_original_query), language="sql", line_numbers=True)
            if result.review_corrected_query:
                st.caption("Reviewed SQL used for execution")
                st.code(format_sql_query(result.review_corrected_query), language="sql", line_numbers=True)
            if result.review_output:
                st.caption("Raw reviewer output")
                st.text(result.review_output)
    elif getattr(result, "review_status", None) == "disabled":
        st.info("🔎 SQL Review Agent is off for this query.")
...
Handler types: {[type(h).__name__ for h in python_logger.handlers]}
    ...
    sql_review = st.checkbox(
        "SQL Review Agent",
        value=False,
        help="Use a second LLM pass to review and correct generated SQL before validation and execution."
    )
